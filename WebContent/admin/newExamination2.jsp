...
<body>
	...
	<div>
		<div style="height:300px;width:180px;float:left;margin-right:20px">
			<details> <summary><a href="admin/NewsExaminationServlet" target = "innerFrame2">所有栏目</a></summary> <%
			List<ModuleNewsType> list = (List<ModuleNewsType>) request.getAttribute("list");
			int size = list.size();
		...
		%> <details> <summary> <a
				href="admin/NewsExaminationServlet"
				target="innerFrame2"><%=columnName%></a> </summary> <%
				for (Map<String, Object> news : listMap) {
			%>
			<div>
				<a
					href="admin/NewsExaminationServlet?type=<%=news.get("newsTypeID")%>"
					target="innerFrame2"><%=news.get("typeName")%></a>
			</div>
			<%
				}
			%> 
		...
		<div>
			<iframe scrolling="yes" src="admin/NewsExaminationServlet"
