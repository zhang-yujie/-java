...
//		System.out.println("actionType:"+actionType);
		if(actionType==null || actionType.equals("")){
			String sql = "select content from modulecolumn where newModuleID = ?";
			
			List<Map<String,Object>> contents = service.cd.executeQuery(sql, new Object[]{Integer.valueOf(moduelId)});
			String getPicSql = "select * from coursegroupmemberphoto";
			//课程简介和课程负责人的内容
			request.setAttribute("contents", contents);
			
			List<Map<String,Object>> pictures = service.cd.executeQuery(getPicSql, new Object[0]);
			request.setAttribute("pictures", pictures);
			
			//课程动态的内容
			String sql2 = "SELECT * from news WHERE type=?";
			List<Map<String,Object>> listMap = service.cd.executeQuery(sql2, new Object[] {21});
...
//							break;
			...
			String sql4 = "select * from courseresoursetype";
			List<Map<String,Object>> resourses = service.cd.executeQuery(sql4, new Object[0]);
			
			request.setAttribute("resourses", resourses);
			
			String sql5 = "select * from links";
			List<Map<String,Object>> links = service.cd.executeQuery(sql5, new Object[0]);
			request.setAttribute("links", links);
			
			request.getServletContext().getRequestDispatcher("/initial.jsp").forward(request, response);
		}
		//首页上的查询
