...
<body>
	...
	<div>
		<div style="height:300px;width:180px;float:left;margin-right:20px">
			<details> <summary><a href="admin/ModuleColumnServlet" target = "innerFrame2">所有栏目</a></summary> <%
		...
		%> <details> <summary> <%if(i == 1){ %> <a
				href="admin/CourseServlet" target="innerFrame2"><%=columnName%></a>
			</summary> <%
			...
			<div>
				<a
					...
					target="innerFrame2"><%=news.get("name")%></a>
			</div>
			<%
				}
			%> </details> <%}else{ %> <a
			...
			<%if(i==0) {%>
				<div>
				<a
					href="admin/CGMPhotoServlet"
					target="innerFrame2">课程资成员</a>
			</div>
			<%} %>
			<%if(i==2) {%>
				<div>
				<a
					href="admin/TeacherTeamServlet"
					target="innerFrame2">教学团队</a>
			</div>
			<%} %>
			<%if(i==4) {%>
				<div>
				<a
					href="admin/CourseResourse"
					target="innerFrame2">课程资源</a>
			</div>
			<%} %>
		...
		<div>
			<iframe scrolling="yes" src="admin/ModuleColumnServlet"
